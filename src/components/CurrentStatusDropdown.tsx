...
function CurrentStatusDropdown({
  show,
  columns,
  ...
  return (
    <div
      className={`bg-white absolute top-16 z-20 left-0 w-full h-[117px] rounded-lg flex-col gap-2 shadow-navM overflow-y-auto dark:bg-darkGrey ${
        show ? "flex" : "hidden"
      }`}
    >
      {columns.map((column) => (
